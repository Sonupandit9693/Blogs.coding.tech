...
const BlogDetails = (ctx) => {
    ...
    useEffect(() => {
      async function fetchComments(){
        const res = await fetch(`${process.env.BASE_URL}/api/comment/${ctx.params.id}`, {cache: 'no-store'})
    ...
    useEffect(() => {
        async function fetchBlog() {
            const res = await fetch(`${process.env.BASE_URL}/api/blog/${ctx.params.id}`, { cache: 'no-store' })
    ...
    const handleLike = async () => {
        try {
            const res = await fetch(`${process.env.BASE_URL}/api/blog/${ctx.params.id}/like`, {
                headers: {
                    'Authorization': `Bearer ${session?.user?.accessToken}`
                },
                method: 'PUT'
    ...
    const handleComment = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.BASE_URL}/api/comment`, {
                headers: {
    ...
    return (
        <div className={classes.container}>
            <div className={classes.wrapper}>
                <Image src={blogDetails?.imageUrl} width='750' height='500' alt=''/>
